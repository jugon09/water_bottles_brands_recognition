...
def test_one_image(image_path):
    path = os.path.join(os.path.join(os.getcwd(), 'core'), 'recognition')
    with open(os.path.join(path, 'model.json'), 'r') as f:
        arch = f.read()
    model = model_from_json(arch)
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=["accuracy"])
    model.load_weights(os.path.join(path, 'Best-weights.hdf5'))
    ...
    test_image /= 255
    test_image = np.expand_dims(test_image, axis=0)
    return names[np.argmax(model.predict(test_image), axis=1)[0]]
